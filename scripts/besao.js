...
var nModule = 15; //total units: 15
arrMLessons = new Array (1,2,3,6,5,4,2,2,8,6,4,2,4,7,2) //lessons in each unit (include the process check for each unit)
arrLessonStatus = new Array(nModule); //array to hold status for each lesson: 0--unread; 1--halfway through; 2--completed
...
//===remove the cookie function by wenqing. instead, use the starttimer when loadPage ================		
...
//end remove
		...
		if (strTemp == "" || typeof(strTemp) == "undefined") { //first time/no data, set up the data
			blnFirstTime = true;
			for (i=0; i<nModule; i++) {
				for (j=0; j<arrLessonStatus[i]; j++) {strTemp += arrLessonStatus[i][j].join() + ",";}//"," --separator for lessons in each unit; 
				strTemp += arrLessonStatus[i].join() + "~";//"~" -- separator for units 	
			} 
			...
			getSuspendData();
			bkmkpage = doLMSGetValue("cmi.core.lesson_location")
...
function getSuspendData() {
	if (verNumber != 2 ) {
		...
		if ( (strTemp != "") && (typeof(strTemp) != "undefined") ) { 
			...
			for (i=0; i<nModule; i++) {
				for (j=0; j<arrMLessons[i]; j++) arrLessonStatus[i][j] = parseInt(arrTemp[i].charAt(2*j));
			}
...
function exitCourse(ExitBtnClicked) {
	...
	if (verNumber != 2) {
//===remove the cookie by wenqing, instead, use computerTime in updateDatabase
